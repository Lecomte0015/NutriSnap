...
export default function PlannerTab() {
  ...
  const addMeal = () => {
    if (!newMealName.trim()) { Alert.alert(t('common.error'), t('manualEntry.errorName')); return; }
  ...
  const deleteMeal = (id: string) => {
    Alert.alert(t('planner.deleteTitle'), '', [
      ...
      { text: t('planner.deleteConfirm'), style: 'destructive', onPress: () => saveMeals(meals.filter(m => m.id !== id)) },
    ]);
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: COLORS.background }]}>
      ...
      <View style={[styles.weekBar, { backgroundColor: COLORS.cardBackground }]}>
        {weekDates.map(d => {
          const cnt = meals.filter(m => m.date === d.date).length;
          const isSelected = d.date === selectedDate;
          return (
            <TouchableOpacity
              key={d.date}
              style={[styles.dayBtn, isSelected && [styles.dayBtnActive, { backgroundColor: COLORS.secondary }]]}
              onPress={() => setSelectedDate(d.date)}
            >
              <Text style={[styles.dayLabel, { color: isSelected ? '#fff' : COLORS.textSecondary }]}>
                {t(`planner.${d.dayKey}` as any)}
              </Text>
              <Text style={[styles.dayNumber, { color: isSelected ? '#fff' : COLORS.textPrimary }, d.isToday && !isSelected && { color: COLORS.secondary }]}>
                {d.label}
              </Text>
              {cnt > 0 && <View style={[styles.dot, { backgroundColor: isSelected ? '#fff' : COLORS.secondary }]} />}
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {mealTypes.map(type => {
          const typeMeals = dayMeals.filter(m => m.mealType === type.key);
          return (
            <View key={type.key} style={[styles.mealSection, { backgroundColor: COLORS.cardBackground }]}>
              <View style={styles.mealSectionHeader}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => { setNewMealType(type.key); setModalVisible(true); }}
                >
              ...
              {typeMeals.length === 0
                ...
                : typeMeals.map(meal => (
                  <View key={meal.id} style={[styles.mealItem, { borderColor: COLORS.border }]}>
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.mealItemName, { color: COLORS.textPrimary }]} numberOfLines={1}>{meal.name}</Text>
                      {meal.calories > 0 && <Text style={[styles.mealItemCal, { color: COLORS.textSecondary }]}>{meal.calories} kcal</Text>}
                    </View>
        ...
        {dayMeals.length === 0 && (
          <View style={[styles.emptyCard, { backgroundColor: COLORS.cardBackground }]}>
            ...
            >
              ...
              <Text style={styles.emptyAddText}>{t('planner.addMeal')}</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>
          ...
          <View style={[styles.modalSheet, { backgroundColor: COLORS.cardBackground }]}>
            ...
            <View style={styles.mealTypeRow}>
              {mealTypes.map(type => (
                <TouchableOpacity
                  key={type.key}
                  style={[styles.mealTypePill, { backgroundColor: COLORS.background, borderColor: COLORS.border }, newMealType === type.key && { backgroundColor: COLORS.secondary, borderColor: COLORS.secondary }]}
                  onPress={() => setNewMealType(type.key)}
                >
                  <Text style={{ fontSize: 14 }}>{type.icon}</Text>
                  <Text style={[{ fontSize: 12, fontWeight: '500' }, { color: newMealType === type.key ? '#fff' : COLORS.textSecondary }]}>{type.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <TouchableOpacity style={[styles.confirmBtn, { backgroundColor: COLORS.secondary }]} onPress={addMeal}>
              <Ionicons name="checkmark-circle" size={20} color="#fff" />
              <Text style={styles.confirmBtnText}>{t('planner.confirm')}</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setModalVisible(false)} style={{ alignItems: 'center', paddingVertical: SPACING.sm }}>
...
const styles = StyleSheet.create({
  ...
  addBtn: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', ...SHADOWS.small },
  weekBar: { flexDirection: 'row', paddingHorizontal: SPACING.sm, paddingVertical: SPACING.sm, marginHorizontal: SPACING.md, borderRadius: BORDER_RADIUS.lg, marginBottom: SPACING.sm, ...SHADOWS.small },
  dayBtn: { flex: 1, alignItems: 'center', paddingVertical: SPACING.sm, borderRadius: BORDER_RADIUS.md },
  ...
  totalBar: { marginHorizontal: SPACING.md, paddingVertical: SPACING.sm, paddingHorizontal: SPACING.md, borderRadius: BORDER_RADIUS.md, marginBottom: SPACING.sm },
  totalText: { fontSize: 14, fontWeight: '600' },
  totalCal: { fontWeight: '800', fontSize: 16 },
  content: { padding: SPACING.md, paddingBottom: 40 },
  mealSection: { borderRadius: BORDER_RADIUS.lg, padding: SPACING.md, marginBottom: SPACING.md, ...SHADOWS.small },
  ...
  addMealBtn: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.sm, paddingVertical: 4, borderRadius: BORDER_RADIUS.round, gap: 4 },
  addMealText: { fontSize: 13, fontWeight: '600' },
  emptyText: { fontSize: 13, fontStyle: 'italic', paddingVertical: SPACING.sm },
  mealItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: SPACING.sm, borderTopWidth: 1, marginTop: 4 },
  mealItemName: { fontSize: 14, fontWeight: '500' },
  mealItemCal: { fontSize: 12, marginTop: 2 },
  deleteBtn: { padding: SPACING.xs },
  emptyCard: { alignItems: 'center', padding: SPACING.xl, borderRadius: BORDER_RADIUS.lg, marginTop: SPACING.md, ...SHADOWS.small },
  emptyCardIcon: { fontSize: 48, marginBottom: SPACING.md },
  emptyCardText: { fontSize: 16, fontWeight: '500', marginBottom: SPACING.xs },
  emptyCardSub: { fontSize: 13, marginBottom: SPACING.lg },
  emptyAddBtn: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, paddingVertical: SPACING.sm, borderRadius: BORDER_RADIUS.round, gap: 6 },
  emptyAddText: { color: '#fff', fontWeight: '700', fontSize: 14 },
  // Modal
  modalOverlay: { flex: 1, justifyContent: 'flex-end' },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' },
  modalSheet: { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: SPACING.lg, paddingBottom: Platform.OS === 'ios' ? 40 : SPACING.lg },
  ...
  mealTypePill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.sm, paddingVertical: 6, borderRadius: BORDER_RADIUS.round, borderWidth: 1.5, gap: 4 },
  modalLabel: { fontSize: 13, fontWeight: '500', marginBottom: 6 },
  modalInput: { borderWidth: 1, borderRadius: BORDER_RADIUS.md, paddingHorizontal: SPACING.md, paddingVertical: 12, fontSize: 15, marginBottom: SPACING.md },
  confirmBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: SPACING.md, borderRadius: BORDER_RADIUS.lg, gap: SPACING.sm, marginBottom: SPACING.sm, ...SHADOWS.small },
